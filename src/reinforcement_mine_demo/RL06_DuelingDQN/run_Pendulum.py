import gym
from src.reinforcement_mine_demo.RL06_DuelingDQN.RL_brain import DuelingDQN
import numpy as np
...
with tf.variable_scope('natural'):
    natural_DQN = DuelingDQN(n_actions=ACTION_SPACE, n_features=3, memory_size=MEMORY_SIZE, e_greedy_increment=0.001,
                             sess=sess, dueling=False)
...
def train(RL):
    ...
    while True:
        ...
        f_action = (action - (ACTION_SPACE) / 2) / ((ACTION_SPACE - 1) / 4)  # [-2 ~ 2] float action
