...
class DuelingDQN(object):
    ...
    # 创建神经网络
    def _build_net(self):
        def build_layers(input, c_name, n_l1, w_initializer, b_initializer):
            with tf.variable_scope('l1'):  # 第一次 两种DQN一样
                w1 = tf.get_variable('w1', [self.n_features, n_l1], initializer=w_initializer, collections=c_names)
                b1 = tf.get_variable('b1', [1, n_l1], initializer=b_initializer, collections=c_names)
                l1 = tf.nn.relu(tf.matmul(input, w1) + b1)

            if self.dueling:
                with tf.variable_scope('Value'):  # 专门分析state的Value，输出只有一个
    ...
    def learn(self):
        ...
        # self.gamma * np.max(q_next, axis=1)这里是乘法不是加法，别写错了，还要检查半天。。。
        q_target[batch_index, eval_act_index] = reward + self.gamma * np.max(q_next, axis=1)
