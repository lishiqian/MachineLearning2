...
class PolicGradient(object):
    ...
    def _build_net(self):
        ...
        all_act = tf.layers.dense(
            inputs=layer,
            units=self.n_actions,  # 神经元个数
            activation=None,  # 之后再加 softmax 激励函数
            kernel_initializer=tf.random_normal_initializer(mean=0, stddev=0.3),
            bias_initializer=tf.constant_initializer(0.1),
            name='fc2'
        )
    ...
    def choose_action(self, observation):
        ...
        # prob_weights.ravel() 扁平化操作,类似于flatten()
        action = np.random.choice(range(prob_weights.shape[1]), p=prob_weights.ravel())  # 根据概率来选 actionS
        return action
    ...
    def _discount_and_norm_reward(self):
        ...
        discounted_ep_rs /= np.std(discounted_ep_rs)  # np.std 计算全局标准差
